...
def _calculate_cell_coverage(idx, affine_trans, geom):
    ...
    x_min, y_min = affine_trans * (col_min, row_max)
    x_max, y_max = affine_trans * (col_max, row_min)
...
if __name__ == "__main__":
   ...
   #feat_poly = unary_union([shape(pol['geometry']) for pol in feat_intersecting])
   geom = MultiPolygon([shape(pol['geometry']) for pol in feat_intersecting])
   print("shown invalid before")
   print(geom.is_valid)
   ...
   #perc_raster[rows, cols] = pool.map(partial(_calculate_cell_coverage, affine_trans=affine_trans, geom=geom), zip(rows, cols))
